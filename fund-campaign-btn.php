    ...
    if ($product->get_type() == 'crowdfunding') {
        if (WPNEOCF()->campaignValid()) {
            ...
            <form enctype="multipart/form-data" method="post" class="cart">
                ...
                <input type="number" min="0" placeholder="<?php echo  get_woocommerce_currency_symbol().' '.$recomanded_price; ?>" 
                name="wpneo_donate_amount_field" 
                class="input-text amount wpneo_donate_amount_field text" value="<?php echo $recomanded_price; ?>" data-min-price="<?php echo $min_price ?>" data-max-price="<?php echo $max_price ?>" >
                <?php do_action('after_wpneo_donate_field'); ?>
                <input type="hidden" value="<?php echo esc_attr($post->ID); ?>" name="add-to-cart"><br>
                <span class="donate_btn_span"><button type="submit" class="<?php echo apply_filters('add_to_donate_button_class', 'wpneo_donate_button'); ?>  donate_btn et_pb_more_button  et_pb_button et_pb_button_one">
                <?php _e('Donate Now', 'wp-crowdfunding'); ?></span>
            </form>
